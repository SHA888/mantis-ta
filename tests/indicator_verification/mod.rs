use mantis_ta::indicators::{
    ATR, AccumDist, BollingerBands, DonchianChannels, EMA, Indicator, MACD, MFI, OBV, ParabolicSar,
    PivotPoints, RSI, SMA, Stochastic, VolumeSMA,
};
use mantis_ta::types::{
    BollingerOutput, DonchianOutput, MacdOutput, PivotOutput, StochasticOutput,
};
...
#[test]
fn verify_donchian() {
    // Reference generated by fixtures/generate_donchian_reference.py, a pure
    // Python reimplementation of rolling MAX(high)/MIN(low). Unlike recursive
    // formulas, windowed max/min is exact and order-independent, so this
    // matches TA-Lib's MAX/MIN functions bit-for-bit without needing the
    // native library installed.
    let candles = load_candles("market_data/spy_daily_5y.csv").unwrap();
    let ref_upper = load_reference_series("reference/donchian_upper_20.json").unwrap();
    let ref_middle = load_reference_series("reference/donchian_middle_20.json").unwrap();
    let ref_lower = load_reference_series("reference/donchian_lower_20.json").unwrap();
    let out: Vec<Option<DonchianOutput>> = DonchianChannels::new(20).calculate(&candles);
    assert_eq!(out.len(), ref_upper.len());
    for i in 0..out.len() {
        match &out[i] {
            Some(d) => {
                assert_close(d.upper, ref_upper[i].unwrap(), "donchian_upper", i);
                assert_close(d.middle, ref_middle[i].unwrap(), "donchian_middle", i);
                assert_close(d.lower, ref_lower[i].unwrap(), "donchian_lower", i);
            }
            None => {
                assert!(
                    ref_upper[i].is_none() && ref_middle[i].is_none() && ref_lower[i].is_none(),
                    "donchian option mismatch at {}",
                    i
                );
            }
        }
...
#[test]
fn verify_accum_dist() {
    // Reference generated by the native TA-Lib `AD` function
    // (fixtures/generate_ad_reference.py) against a dedicated dataset where
    // close is NOT pinned to the high/low midpoint (unlike spy_daily_5y.csv,
    // where the money-flow-multiplier is ~0 on every bar and can't
    // discriminate a correct AD implementation from a broken one). A/D is
    // cumulative and unbounded, but it uses the same money-flow-multiplier
    // expression accumulated in the same bar order as TA-Lib, so the two
    // series agree to well within EPS.
    let candles = load_candles("market_data/ad_parity_synthetic.csv").unwrap();
    let reference = load_reference_series("reference/ad.json").unwrap();
    let out: Vec<Option<f64>> = AccumDist::new().calculate(&candles);
    assert_series(&out, &reference, "ad");
}

...
fn verify_accum_dist() {
    ...
    // (fixtures/generate_ad_reference.py). A/D is cumulative and unbounded, but
    // it uses the same money-flow-multiplier expression accumulated in the same
    // bar order as TA-Lib, so the two series agree to well within EPS.
    let candles = load_candles("market_data/spy_daily_5y.csv").unwrap();

...
fn verify_wma_consistency() {
    let prices = vec![44.34, 44.09, 44.15, 43.61, 44.33, 44.83, 45.10, 45.42, 45.84, 46.08, 45.89, 46.03, 45.61, 46.28, 46.00, 46.00, 46.00, 46.00, 46.00, 46.00];
...
fn verify_tema_consistency() {
    let prices = vec![44.34, 44.09, 44.15, 43.61, 44.33, 44.83, 45.10, 45.42, 45.84, 46.08, 45.89, 46.03, 45.61, 46.28, 46.00, 46.00, 46.00, 46.00, 46.00, 46.00, 46.00, 46.00, 46.00, 46.00, 46.00, 46.00, 46.00, 46.00];
    let candles = create_candles(&prices);
...
fn verify_cci_consistency() {
    let highs = vec![44.34, 44.09, 44.15, 43.61, 44.33, 44.83, 45.10, 45.42, 45.84, 46.08];
    let lows = vec![44.34, 44.09, 44.15, 43.61, 44.33, 44.83, 45.10, 45.42, 45.84, 46.08];
...
fn verify_adx_consistency() {
    let highs = vec![44.34, 44.09, 44.15, 43.61, 44.33, 44.83, 45.10, 45.42, 45.84, 46.08, 45.89, 46.03, 45.61, 46.28, 46.00, 46.50, 46.75, 47.00, 47.25, 47.50, 47.75, 48.00, 48.25, 48.50, 48.75, 49.00, 49.25, 49.50];
    let lows = vec![44.34, 44.09, 44.15, 43.61, 44.33, 44.83, 45.10, 45.42, 45.84, 46.08, 45.89, 46.03, 45.61, 46.28, 46.00, 46.25, 46.50, 46.75, 47.00, 47.25, 47.50, 47.75, 48.00, 48.25, 48.50, 48.75, 49.00, 49.25];
    let closes = vec![44.34, 44.09, 44.15, 43.61, 44.33, 44.83, 45.10, 45.42, 45.84, 46.08, 45.89, 46.03, 45.61, 46.28, 46.00, 46.38, 46.63, 46.88, 47.13, 47.38, 47.63, 47.88, 48.13, 48.38, 48.63, 48.88, 49.13, 49.38];
    let candles = create_ohlc_candles(&closes, &highs, &lows, &closes);
...
fn verify_streaming_equals_batch() {
    let prices = vec![44.34, 44.09, 44.15, 43.61, 44.33, 44.83, 45.10, 45.42, 45.84, 46.08, 45.89, 46.03, 45.61, 46.28, 46.00];
    let candles = create_candles(&prices);
    ...
    for (i, (batch, streaming)) in batch_outputs.iter().zip(streaming_outputs.iter()).enumerate() {
        match (batch, streaming) {
            (Some(b), Some(s)) => {
                assert!((b - s).abs() < 1e-10, "Mismatch at index {}: batch={}, streaming={}", i, b, s);
            }
            (None, None) => {}
            _ => panic!("Mismatch at index {}: batch={:?}, streaming={:?}", i, batch, streaming),
        }
    }
}

...
### Added
- **Parabolic SAR indicator**: New `ParabolicSar` streaming Wilder's SAR (Stop And Reverse) indicator with configurable acceleration factor (`af_start`/`af_step`/`af_max`), O(1) `next()`, `warmup_period() == 2`, and `reset()`. Verified against the real TA-Lib C library (`talib.SAR`, not the Python reformulation used for other Tier 2 indicators) to 1e-9 over a 2000-bar SPY fixture (`fixtures/reference/sar.json`, generated by `fixtures/generate_sar_reference.py`).

### Fixed
- **Parabolic SAR reversal clamp**: On a trend reversal, the new SAR is now correctly clamped to `max(extreme_point, current_bar_high, prior_bar_high)` (and the mirrored case for downtrend→uptrend), matching TA-Lib's behavior. Previously the new SAR could land inside the reversal bar's own high/low range.
- **Parabolic SAR bootstrap clamp**: Fixed the warmup-boundary seeding so the first recurrence step clamps only against the immediately preceding bar, not the very first (seed) bar, matching TA-Lib's bootstrap behavior.
